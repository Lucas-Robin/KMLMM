...
def larger_model(input_shape=(1, 16, 16), num_classes=10):
	# create model
	model = Sequential()
...
def my_model(input_shape, num_classes=10):
    # create model
	  model = Sequential()
...
def lenet5(input_shape, num_classes=10):
    model = Sequential()
    model.add(Convolution2D(6, 5, 5, border_mode='valid', input_shape=input_shape, activation='relu'))  # layer C1
    model.add(MaxPooling2D(pool_size=(2, 2)))  # layer S2
    model.add(Convolution2D(16, 5, 5, border_mode='valid', input_shape=input_shape, activation='relu'))  # layer C3
    model.add(MaxPooling2D(pool_size=(2, 2)))  # layer S4
    model.add(Convolution2D(120, 5, 5, border_mode='valid', input_shape=input_shape, activation='relu'))  # layer C5
...
def main():
    ...
    # Fit the model
    model.fit(X_train, y_train, validation_data=(X_test[:n_test, :, :, :], y_test[:n_test]), nb_epoch=10, batch_size=200, verbose=2)
